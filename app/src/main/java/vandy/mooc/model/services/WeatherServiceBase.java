...
public class WeatherServiceBase 
    ...
    /**
     * Appid needed to access the service.
     * TODO -- Done.
     */
    private final String mAppid = "364f800b2a2773a7d560924544006fb7";
    ...
    protected List<WeatherData> getWeatherResults(String location) {
        ...
        // checking results from cache
        if (null != results) {
            Log.d(TAG,
                    "Getting results from cache for "
                            + location);
            ...
            Log.d(TAG,
                    "Getting weather from service for "
                    + location);
            // wie don't have weather for this location in cache or
            // info in cache is old, so trying obtain this from service
            results = getResultsFromWeatherService(location);

            // Store result in cache for up to
            // DEFAULT_CACHE_TIMEOUT time with key=location
            GenericSingleton.instance(WeatherCache.class)
                    .put(location, results, DEFAULT_CACHE_TIMEOUT);
            return results;
        }
    }
